...
// 法定福利費（福利厚生費）の下限を保証する。
// AIが「労務費の4%」のような誤った率を書いてくることがあり、そのまま出すと下請けの見積として成立しない。
// 建設業の事業主負担は 健保約5.0% + 介護約0.8% + 厚年9.15% + 雇用約0.95% + 労災約0.95% ≒ 16%。
// 率を作文させず、15%を下回ったらコード側で16%に引き上げる（元請から社会保険加入を厳しく見られる費目のため）。
const WELFARE_MIN_RATE = 0.15;
const WELFARE_RATE = 0.16;
function enforceStatutoryWelfare(result: any, context: string, fallbackMarkup: number): string[] {
  // 遮熱シート(山下さん)は材工共の㎡単価に法定福利まで込みで、別立てしない仕様（stripHeatshieldDoubleCounted で除去済み）
  if (result?.isHeatshield) return [];
  const rows: any[] = result.breakdown;
  ...
  const laborBase = rows
    .filter((b) => classifyBreakdownItem(b) === '施工費')
...
function reconcileEstimateTotal(result: any, context: string, fallbackMarkup = DEFAULT_MARKUP): any {
  ...
  const warns = [
    ...
    ...enforceStatutoryWelfare(result, context, fallbackMarkup),   // ★行を直し切ってから、法定福利費の下限(労務費の15%)を保証
  ];
...
const TAKEOFF_FACTOR_DEFAULTS = {
  ...
  curtainBox: 0,          // カーテンボックス内部を貼るなら1、貼らないなら0
};
...
金額は数値のみ（カンマや円記号は除去）。日付はYYYY-MM-DD形式に変換。`
  ...
  const analyzeImageCore = async (data: any) => {
    ...
    const hasPdfInput = String(imageBase64 || '').startsWith('data:application/pdf')
      || extraShots.some((x: string) => String(x).startsWith('data:application/pdf'));
    const opName = isBeforeAfter ? 'ビフォーアフター見積'
      : hasPdfInput ? '図面PDF見積'
      : hasImageInput && hasCommentInput ? '写真+コメント見積' : hasImageInput ? 'AI見積' : 'テキスト見積';
    // クレジットチェック
    await syncRemoteLicense(false);
    ...
    // ★Web検索は数分かかるので絶対に await しない。キャッシュがあれば使い、
    //   無ければ待たずに背景で温めて今回はスキップ（次回の見積から市場情報が乗る）。
    let marketPrompt = '';
    try {
      const cachedInsight = readMarketInsightCache({ location, comment, industryType });
      if (cachedInsight) {
        marketPrompt = buildMarketPrompt(cachedInsight);
      } else {
        warmMarketInsight(config.anthropicKey, { location, comment, industryType });
      }
...
- ★★最優先ルール：自社実績があれば必ずその㎡単価・総額に合わせる★★
  上の「自社実績価格アンカー」「過去に読み取った自社書類」「修正履歴」に同じ工法の実績があるなら、その実額が絶対基準。
  実績が¥6,500/m²ならその近辺、¥5,000/m²なら¥5,000近辺で出す。実績があるのに下の市場レンジや「特許プレミアム」を理由に
  実績から大きく離れた（＝数倍の）金額を出すことは誤り。実績を無視して高く盛らないこと。
- ★実績が無い/薄い工法のときの初期値（材工込み・本体シート施工のみ。足場・安全・諸経費は別途計上）:
  ①下限(3,000〜4,000円/m²)は汎用シート水準なので本体には使わない。
  ②初期値は中心帯 5,000〜6,500円/m² を基準にする。高い側の条件（下記）が複数重なるときだけ 7,000〜10,000円/m²。
  ③★本体の材工込み㎡単価は上限 10,000円/m²。自社実績がこの上限を超える場合のみ実績に従い、それ以外で10,000円/m²を超えて出してはならない。
  confidence は必ず「高/中/低」の文字で返す（数値は不可）。実績が薄ければ「低」、recommendations に「実績を入れていただくほど御社の工法別の金額帯に合います」と添える。
...
- ★"高い側(6,000〜10,000円/m²)" に寄る条件（複数当てはまる場合のみ。上限は10,000円/m²）:
...
- ★★★折板屋根の「見積数量」は屋根面積そのものではない★★★
  折板は山谷があるので、実際に張るシートの面積は屋根面積より大きい。
...
`;
    ...
    // 業種別のAI指示
    const industryPrompt = industryType === 'building'
      ? `\n## ★業種: 建築一式工事業（新築・増改築の元請）★
この会社は建築一式工事（新築・増改築）を元請で請ける会社です。単一の専門工事ではなく、**建物全体を工事科目に分けて積み上げる内訳書**を作ってください。以下を厳守:

【必ず工事科目（大項目）ごとに分けて積算する】
建物の全体見積は、次の科目を漏れなく検討し、その工事に該当する科目だけを breakdown に立てること（該当しない科目は省く）:
① 仮設工事（共通仮設＝現場事務所・仮設電気水道・安全対策／直接仮設＝足場・養生・墨出し）
② 基礎・地業工事（土工・根切り・残土処分・地業・杭・鉄筋・型枠・基礎コンクリート）
③ 躯体工事（木造＝土台・建方・軸組・金物／鉄骨造＝鉄骨建方・デッキ／RC造＝躯体鉄筋・型枠・打設）
④ 木工事・造作（間柱・下地・造作材・階段）
⑤ 屋根・防水工事（屋根葺き・板金・樋・バルコニー防水）
⑥ 外装工事（外壁サイディング／左官／タイル・シーリング）
⑦ 断熱・気密工事
⑧ 建具・サッシ・ガラス工事（外部建具・内部建具・玄関）
⑨ 内装工事（ボード・クロス・床フローリング・天井・畳）
⑩ 塗装工事
⑪ 電気設備工事（幹線・配線・器具・盤・弱電）
⑫ 給排水衛生設備工事（給水・排水・給湯・衛生器具・キッチン・浴室UB）
⑬ 空調・換気設備工事
⑭ 外構・造園工事（駐車場・アプローチ・フェンス・門・植栽）
⑮ 諸経費（現場管理費・一般管理費）※下記で必ず計上

【坪単価で妥当性を必ず検証（構造別）】
延床坪数が分かる（または図面・写真から推定できる）場合、構造別の坪単価レンジで総額の妥当性をチェックし、極端な乖離があれば修正すること:
- 木造 新築: おおむね 60〜90万円/坪（グレードで変動）
- 鉄骨造(S造) 新築: おおむね 80〜120万円/坪
- RC造 新築: おおむね 90〜130万円/坪
- 増改築・改修は工事範囲で大きく変わるため、坪単価は参考に留め、科目別の積み上げを優先。
※これは概算レンジ。自社の過去実績・修正履歴があれば必ずそちらを最優先にする。

【元請の諸経費を必ず入れる】
元請は下請各社の工事原価に加えて、現場管理費（現場監督人件費・安全・仮設光熱費等）と一般管理費（会社経費・利益）を必ず計上する。目安は工事原価に対して現場管理費＋一般管理費で合計 15〜25% 程度（規模が小さいほど率は高め）。これを category="経費" の行として立て、costBase と cost（利益込み）を分けること。

【元請ならではの注意】
- 各科目は「専門工事会社への外注（下請）」が基本。breakdownの note には「◯◯工事一式（下請）」＋数量根拠（延床◯坪・外壁◯㎡等）を書く。
- 建築確認申請・検査・地盤調査・給排水引込・付帯（照明器具・カーテン・エアコン）等の**別途工事・施主支給の扱い**を recommendations に明記し、見積の前提（含む／含まない）をはっきりさせる。
- 図面（平面・立面・仕様書）が無く写真・概要だけの場合は、坪数と構造の仮定を estimatedScale と breakdown の note に明記し、confidence を下げること。\n`
      : industryType === 'lease'
      ? `\n## ★業種: 仮設工事リース業★
...
【最重要: プラントは写真だけで積算できない】
- プラント配管の多くは口径・材質・肉厚・溶接継手数で金額が桁違いに変わり、写真には全体が写らない。**P&ID・アイソメ図・機器リスト・配管仕様書(Line List)が無いと正確な積算は不可能**。図面・資料が無い場合は、breakdownで前提（口径・材質・数量の仮定）を明記したうえで概算を出し、recommendationsに「アイソメ図・機器リスト・配管仕様（口径/材質/Sch）をご提供いただくと精度が上がります」と書き、confidenceを必ず「低」または「中」にすること。安く出しすぎない。

【工種を必ず分けて別項目で積算する】
① 配管工事: 口径×材質×長さ(m) ＋ 溶接継手数。材質で単価が大きく違う（SGP/配管用炭素鋼 < ステンレスSUS304/316 < 合金鋼/高圧配管）。Sch（肉厚）・保温の有無・サポート（架台・振れ止め）も計上。溶接は「箇所数×口径係数」で溶接工の人工を積む。
② 機器据付: タンク・圧力容器・熱交換器・ポンプ・塔槽・ブロワ等。据付は「重量物揚重（クレーン規模×日数）＋アンカー・基礎・レベル出し＋芯出し調整」。機器本体が支給か込みかを必ず区別（支給なら据付費のみ）。
③ 保温・保冷・耐火被覆: 配管・機器の表面積×厚み。保温材（ロックウール/グラスウール/けい酸カルシウム）＋ラッキング（SUS/アルミ外装）。
④ 計装・電気: 計器・センサー・制御盤(DCS/PLC)・ケーブルラック・電線管。**防爆仕様(耐圧防爆/本質安全防爆)か否かで単価が変わる**ので必ず確認事項に入れる。
⑤ 鉄骨・架構: パイプラック・機器架台・ステージ・歩廊。
⑥ 検査・試験: 非破壊検査(RT/UT/PT/MT)・耐圧試験・気密試験・溶接検査。プラントでは省略不可の費用。
⑦ 仮設・足場: 高所・狭所・タンク内の足場。重量物揚重の大型クレーン回送・組立。

【労務単価は建築より高い（有資格職）】
- 配管工/溶接工（アーク・TIG、有資格者）、据付工（重量鳶）、計装工。単価は電気設備より高めに置く。溶接は資格区分（PQR/WPS準拠）で歩掛が変わる。
...
【最重要: 内装仕上の範囲だけを積算する。建物全体を絶対に積算しない】
- **躯体・基礎・外装・屋根・防水・外部建具・電気設備・給排水設備・空調・外構は範囲外**。1円も計上してはいけない。これらを入れると金額が桁で狂う。
- 元請ではないので**建築確認申請・共通仮設（現場事務所・仮設電気水道）・外部足場は計上しない**（元請＝ゼネコン持ち）。自社で持つのは室内の脚立・立馬・養生・残材処分まで。
- 現場管理費もゼネコン側が持つ。自社の経費は**一般管理費として工事原価の8〜15%程度**に留める（元請の15〜25%より低い）。

【数量拾いの手順（内装屋の拾い方に合わせる）】
1. **まず平面図から「部屋ごとの床面積」を室名つきで全部拾う**（例: 居室1 13.17㎡／食堂・談話室 76.70㎡／廊下／厨房…）。内装の拾いはここが本体。breakdown の note に室名と㎡を必ず残すこと。
2. **壁は「室内周長」から出す。床面積に固定倍率を掛けるな。**
   - 各室の内周長を出す（正方形に近い室なら 周長 ≒ 4×√(その室の床面積)。細長い室は縦横から実測）。
   - **壁延長 L ＝ 全室の内周長の合計 ＋ 廊下の両側延長**。間仕切りは両面に仕上げがあるので、各室の内周を足し込めば自動的に両面が入る。
   - **壁面積 ＝ L × 天井高**。
   - 検算: 壁面積 ÷ 床面積 の倍率は、**大部屋が多い建物で1.8〜2.3、個室主体で2.6〜3.2**が目安。この範囲から大きく外れたら室の拾い落としを疑う。**倍率だけで壁面積を決めてはいけない**（大部屋の比率で倍率は大きく変わる）。
   - 建具・窓の開口は、内装の慣行として**小開口は控除しない**（控除するのは概ね1箇所0.5㎡超のまとまった開口のみ）。
3. **天井面積 ≒ 床面積 × 0.9〜1.0**。
4. ★用語を固定する。以後この定義以外で数量を出すな★
   - **W ＝ 壁仕上げ面積**（＝上で出した「全室の内周長の合計 × 天井高」。間仕切りの両面が入った数字）
   - **C ＝ 天井面積**、**H ＝ 天井高**
   - この W と C から、各工種の数量を**次の式で機械的に出す**こと:
     * **せっこうボード ＝ W ＋ C**（仕上げ面ごとに張るため、Wをそのまま使う）
     * **クロス ＝ W ＋ C**（同上）
     * **LGS軽天下地 ＝ W ÷ 2 ＋ C**（1組の下地が間仕切り両面のボードを受けるため、必ず半分にする。**ボードと同じ数量にしたら誤り**）
     * **幅木 ＝ W ÷ H × 0.9**（仕上げ面の足元すべてに付く。開口部で10%控除。**W÷H÷2 にしたら半分になって誤り**）
   - note には必ず「W＝◯㎡、C＝◯㎡」と明示し、各行がこの式から出ていることを示せ。**軽天とボードが同じ数量になっていたら、その時点で誤りなので出し直せ。**
   - **ボードの一重／二重を必ず判定する**。病院の病室・老人ホームの居室間仕切り・会議室等の**遮音を要する区画は12.5mm二重張りが標準**。二重なら該当区画のボード数量を2倍にする。仕上表が無く判断できない場合は「一重で計上」した上で、recommendations に「居室間仕切りは遮音上、二重張りの可能性があります。仕上表でご確認ください（二重の場合 約◯◯万円増）」と**増額幅つきで**明記すること。
6. **天井高が図面に無い場合は仮定を明記**し、estimatedScale と note に「天井高◯mと仮定」と書き、confidence を下げること。

【工種を必ず分けて別項目で計上する】
① 軽量鉄骨下地（LGS軽天）: 壁下地・天井下地を分ける
② せっこうボード張り（PB）: 9.5/12.5、一重張り・二重張り、耐水・耐火の別
③ 壁・天井仕上: ビニールクロス／布・特殊クロス／岩綿吸音板／化粧ボード
④ 床仕上: 長尺シート／タイルカーペット／Pタイル／フローリング／タイル／畳／OAフロア（＋下地調整・セルフレベリング）
⑤ 副資材・付属: 幅木・見切り・コーナー材・カーテンレール・ブラインド・ガラスフィルム
⑥ 雑工事: 既存撤去、養生、残材処分、清掃
※その現場に無い工種は立てない。**軽天ボードを他社が施工する現場もある**ので、範囲が不明なら recommendations に「軽天・ボード下地は御社施工範囲でしょうか」と確認事項を書く。

【単価の目安（下請施工単価・材工共・2026年）※自社実績があれば必ずそちらを優先】
- LGS軽天 壁下地 1,800〜2,800円/㎡ ／ 天井下地 1,600〜2,500円/㎡
- PB張り 一重 1,200〜1,800円/㎡ ／ 二重張り 2,200〜3,200円/㎡
- ビニールクロス（下地パテ処理込）1,000〜1,500円/㎡ ／ 布・不燃特殊クロス 1,800〜3,500円/㎡
- 岩綿吸音板 1,800〜2,800円/㎡
- 長尺シート 4,000〜6,500円/㎡ ／ タイルカーペット 3,500〜5,000円/㎡ ／ Pタイル 2,500〜3,500円/㎡
- フローリング 5,000〜9,000円/㎡ ／ OAフロア 6,000〜10,000円/㎡
- 幅木 500〜900円/m ／ ブラインド 8,000〜15,000円/箇所 ／ ガラスフィルム 6,000〜12,000円/㎡
- 労務: 内装工・表具工 22,000〜28,000円/人日 ／ 軽天工・ボード工 23,000〜30,000円/人日

【★この業種で絶対にやってはいけないこと（違反したら出し直せ）★】
1. **壁面積を「床面積 × 天井高」から出すな。**床面積(㎡)に高さ(m)を掛けても壁面積にはならない（次元が合わない）。壁は必ず「全室の内周長の合計 L(m) × 天井高 H(m)」＝W で出し、note に L の値を書け。
2. **施工費の行の単位を「式」にするな。**必ず単位＝「人工」、数量＝人工数で出せ（単価＝1人工あたりの売価）。**1行に複数の工種を詰め込むな**（LGS下地／ボード張り／クロス張り／床張りは別の行にする）。
3. **法定福利費は「労務費原価 × 16%」で出した額をそのまま計上しろ。**算出後に「実費ベース調整」等の名目で減額するな。
4. **せっこうボードの3×6板は 910×1820mm ＝ 1.656㎡** である（1.82㎡ではない）。㎡単価を板単価から割り戻すときはこの値を使え。
5. **材料単価は「業者の仕入原価」で置け。カタログ定価やホームセンター価格を使うな。**主要材料の材料のみ原価の実勢は次の通り。これを大きく超えたら高すぎるので見直せ:
   - ビニルクロス（量産〜1000番・抗菌品含む）: **300〜500円/㎡**（糊・副資材込み）
   - せっこうボード 12.5mm: **380〜520円/㎡** ／ 9.5mm: **320〜450円/㎡**
   - LGS壁下地材（スタッド・ランナー・振れ止め）: **700〜1,100円/㎡** ／ LGS天井下地材（野縁・野縁受け・ハンガー）: **900〜1,400円/㎡**
   - 防滑性ビニル床シート t2.0: **2,200〜3,000円/㎡** ／ 長尺塩ビシート t2.5: **2,400〜3,200円/㎡**（いずれも接着剤・溶接棒込み）
   - ビニル幅木 H60: **300〜450円/m**
6. **過去実績による材料費の補正倍率が2倍（+100%）を超える場合は、そのまま適用するな。**既存改修の実績（撤去・下地補修・不陸調整を含む）を新築に当てると過大になる。新築案件では上記5の実勢レンジを優先し、補正は参考に留めて recommendations にその旨を書け。
7. **人工は「延べ人日」である。歩掛から算出した人工を、後から減らすな。**「並行作業で圧縮」「複数名の効率化により短縮」といった理由で人工を減らすのは誤り（12人工の仕事を3人でやれば4日かかるだけで、6人工にはならない）。**数量 ÷ 歩掛 ＝ 人工 をそのまま計上しろ。**「何人×何日」はその人工の内訳として書くだけで、掛け算の答えは必ず算出した人工と一致させろ。
8. **壁面積を「床面積 × 一定の倍率（×2.0 等）」で決めるな。**必ず室内周長の合計 L から W ＝ L × H で出し、note に L の値と、主要な室の周長の内訳を書け。倍率はあくまで出した後の検算に使う数字である。
9. **note に「過去実績 +◯%補正を反映」と書くな。**それは社内の学習事情であって、客に見せる単価の根拠にならない。単価の根拠は「材料の構成と単価」「歩掛」「労務単価」で説明しろ。

【歩掛は工種ごとに変える（全部同じ歩掛にするな）】
- LGS軽天下地組み: 60〜80㎡/人日 ／ せっこうボード張り: 55〜70㎡/人日
- **クロス張りは軽天・ボードより速い: 90〜120㎡/人日**（下地パテ処理込みでも80〜100㎡/人日）
- 床シート張り（熱溶接・端部処理込み）: 20〜30㎡/人日 ／ 幅木取付: 100〜140m/人日
※LGS・ボード・クロスに同一の歩掛を当てると人工が狂う。必ず上記の幅から工種ごとに選び、note に「◯㎡ ÷ ◯㎡/人日 ＝ ◯人工」と書け。

【積算の自己検算（breakdown を出す前に必ず実施）】
- 「福利厚生費（法定福利費）」の算定に使う労務費原価は、**breakdown の category="施工費" 各行の原価の合計と必ず一致させる**。note に内訳を並べたら、その足し算が書いた合計と合っているか検算してから書け（並べた数字の和と違う総額を書くのは誤り）。

【建物用途で単価が変わる（必ず判定する）】
- **病院・製薬・研究所・クリーンルーム**: 抗菌／防カビ／帯電防止／不燃仕様、床シートの**熱溶接接合**、幅木の**立上げRコーナー**、無塵・防塵養生。→ 標準より**2〜3割増**。仮設養生と工程調整（稼働中の病棟は夜間・区画施工）も別途計上。
- **老人ホーム・福祉施設**: 手すり下地の合板補強、防滑性ビニル床シート、抗菌クロス、車椅子対応の見切り。→ 標準より**1〜2割増**。
- **オフィス・学校・商業**: 標準レンジ。OAフロア・タイルカーペットが主。
- **マンション・戸建**: 量産クロス（1,000円/㎡前後）・複合フローリングが主で、最も安い帯。

【総額の妥当性チェック（必ず実施）】
★主チェック＝**仕上げ面積あたり**（＝壁面積＋天井面積＋床面積）。延床あたりの単価は大部屋の比率で大きくぶれるので、こちらを主に使う:
- **仕上げのみ（クロス・床・幅木、下地は他社）: 仕上げ1㎡あたり 2,000〜4,000円**
- **軽天・ボード下地まで含む: 仕上げ1㎡あたり 5,000〜9,000円**
副チェック＝延床あたり（幅は広い。参考程度）:
- 仕上げのみ 延床1㎡あたり 7,000〜18,000円 ／ 軽天ボード込み 延床1㎡あたり 18,000〜45,000円
※病院・製薬・研究所は上振れ、マンション・戸建は下振れ。**延床1㎡あたり10万円を超える場合は、建物全体（躯体・外装・設備）を積算してしまっている疑いが濃厚**なので、必ず範囲を見直すこと。

【提案に必ず書くこと】
- 見積の範囲（含む工種／含まない工種）をはっきり書く。特に**軽天・ボードの有無、既存撤去の有無、養生・残材処分の負担区分**。
- 夜間・土日作業、稼働中施設の区画施工、他業者との工程調整が必要な場合は割増の可能性を明記。
- 図面が平面図のみで**天井伏図・仕上表（内装仕上げ表）が無い場合は、仕様の仮定を明記**し「仕上表をご提供いただくと精度が上がります」と recommendations に書き、confidence を下げること。\n`
...
ホームセンター等の汎用遮熱シート・断熱材・アルミ保温材の安い相場（数百〜千円/m²程度）を本体に絶対に当てはめないこと。本体は「この会社の過去実績（上記★実績アンカー・修正履歴・自社の金額帯）」、特に同じ工法の実績を最優先の基準にする。実績がまだ無ければ特許プレミアム商材として専門単価で見積もるが、**本体の材工込み㎡単価は上限10,000円/m²（初期値は5,000〜6,500円/m²）を厳守し、これを超えて見積もらないこと**（自社実績が上限超えを示す場合のみ実績に従う）。実績が無い場合はconfidenceを「低」（数値でなく必ず高/中/低の文字）にし、recommendationsに「実績を入力いただくほど御社の工法別の金額帯に合った精度になります」と記載すること。
...
- 【施工指示は公的基準に準拠し厳格化】installInstructionは下記の【公的基準・法令リファレンス】に必ず準拠し、該当する基準名（例: 安衛則518〜521条／フルハーネス／JIS A 6514／建築物石綿含有建材調査者による事前調査／通気層確保 等）を施工指示の各行に明記すること。工法に応じて、①墜落防止（屋根上作業を伴うスカイ工法・カバー工法では必須。2m以上・フルハーネス・親綱・折板の踏み抜き注意）、②石綿事前調査（既存屋根がスレート等の改修・とくにカバー工法では必須）、③通気層の確保（全工法共通。屋根下工法は小屋裏換気とセット）、を必ず含めること。${HEATSHIELD_PRICE_NOTE}${HEATSHIELD_STANDARDS}\n`
    ...
    if (isBeforeAfter) {
      ...
      shots.forEach((f: string, i: number) => {
        // PDF（ゼネコンから来る図面はたいていPDF）はそのまま document として渡す。
        // Claude はPDFを直接読めるので、画像に変換する必要はない。
        const isPdf = String(f).startsWith('data:application/pdf');
        if (shots.length > 1) userContent.push({ type: 'text', text: `［${isPdf ? '資料' : '写真'}${i + 1}］` });
        if (isPdf) {
          userContent.push({
            type: 'document',
            source: { type: 'base64', media_type: 'application/pdf', data: String(f).replace(/^data:application\/pdf;base64,/, '') },
          });
        } else {
          userContent.push({
            type: 'image',
            source: { type: 'base64', media_type: detectMediaType(f), data: String(f).replace(/^data:image\/\w+;base64,/, '') },
          });
        }
      });
    }
...
` : `## 建物構造（未指定 → 必ず推察せよ）
構造が指定されていない。画像・図面・工事内容・外壁材や軒裏・柱の見え方から、木造・鉄骨造・RC造・SRC造のいずれかを推察し、estimatedScale に「（推定）◯◯造」の形で記載してから、その構造前提で解体・撤去・施工手間を積算しろ。判断材料が皆無のときだけ「構造不明」とせよ。
`}
${(buildingAge && String(buildingAge).trim() && Number(buildingAge) > 0) ? `## ★築年数（改修・修繕・解体のコストに必ず反映）★
築${String(buildingAge).trim()}年。
★重要: 新築でない工事では築年数で以下を織り込め（新築工事では無視してよい）。
　ただし下記はあくまで一般論だ。**修正履歴に同じ構造・近い築年帯の自社実績があれば、必ずそちらを優先しろ**:
- 築20年超: 既存撤去・下地補修の手間増。「開けてみないと分からない」劣化ぶんの現実的な余裕を見込め。
- 築30年超: アスベスト(石綿)含有の可能性。recommendations に事前調査(レベル別)・除去の要否を必ず触れろ。基礎・躯体の劣化点検も。
- 築40年超(≒1981年以前の旧耐震相当): 耐震補強の前提が変わる点を recommendations で触れろ。
割増は「見えない劣化への現実的な余裕」であり、根拠を note / recommendations に必ず書け（過剰計上はするな）。
` : `## 築年数（未入力 → 内外装の年代感から推察して逆算せよ）
築年数が入力されていない。画像がある場合、内外装のパターンから築年帯を推察して逆算しろ:
- サッシ(アルミ単板/樹脂/木製)、外壁材(モルタル・リシン/窯業サイディング世代/ALC/タイル)、屋根材、玄関・建具の意匠、水回り設備・タイル・照明の年代感、クロス・床材の世代。
- そこから「概ね築◯〜◯年」を推定し estimatedScale に「（推定）築◯年前後」と記載。改修・解体ならその築年帯に応じた既存撤去・下地補修・石綿・基礎点検の余裕を上と同じ基準で織り込め。
- 新築や、年代を判断できる材料が無い場合は築年数を無視してよい（無理に推定しない）。
`}
${(() => {
  const sc = siteConditions || {};
  const filled = [sc.access, sc.adjacency, sc.occupied, sc.stories].filter((v: any) => v && String(v).trim());
  if (filled.length > 0) {
    return `## ★現場条件（お客様確認済み — 足場・搬入・段取りに必ず反映）★
${sc.access && String(sc.access).trim() ? `- 搬入・道路: ${String(sc.access).trim()}\n` : ''}${sc.adjacency && String(sc.adjacency).trim() ? `- 隣地・足場: ${String(sc.adjacency).trim()}\n` : ''}${sc.occupied && String(sc.occupied).trim() ? `- 居住状況: ${String(sc.occupied).trim()}\n` : ''}${sc.stories && String(sc.stories).trim() ? `- 階数・高さ: ${String(sc.stories).trim()}\n` : ''}★重要: これらは写真に写らないが金額を直撃する現場条件だ。以下を必ず積算に反映しろ（過剰計上はするな、根拠を note / recommendations に書け）:
- 搬入が悪い（狭あい道路・車両進入困難・旗竿地）ほど材料の小運搬・荷揚げ手間と搬入車両の割増を見込め。
- 隣地が近い・越境・特殊足場ほど足場の組立手間・養生・特殊足場費を見込め。四周に余裕があれば標準足場でよい。
- 居ながら施工・営業中施工は養生範囲拡大・時間帯制約・近隣対応・区画割りの手間を見込め。空き家なら制約は少ない。
- 階数・高さが上がるほど足場㎡・高所作業・危険手当を見込め。4階建て以上は特殊足場・高所割増。`;
  }
...
6. ★必須★ breakdownには以下の3項目を必ず最後に含めろ（2025-2026年の建設業界情勢を反映した現実的な金額にすること。資材高騰・人件費上昇・働き方改革による人手不足を考慮）:
   ...
   - 「福利厚生費」（法定福利費・社会保険・雇用保険・退職金積立等。**人件費（労務費）原価の15〜20%**。2024年問題で上昇中。最低でも3万円以上）
     ★率を自分で作るな。法定福利費の事業主負担は「健康保険 約5.0% ＋ 介護保険 約0.8% ＋ 厚生年金 9.15% ＋ 雇用保険 約0.95% ＋ 労災保険 約0.95%」＝**合計およそ16%**。ここに退職金共済(建退共)等を加えて15〜20%になる。**5%未満や10%未満の率を書いたら誤り**。必ず労務費原価×15%以上で計上し、note に「労務費◯◯円 × 16% ＝ ◯◯円」と式を書け。
...
10. ★松竹梅（gradeOptions）＝ 提案用の3グレード★
   - 竹 = この見積そのもの。竹.total は estimatedTotal と必ず一致させ、diff は 0。ベースの breakdown・金額は松竹梅のために一切いじるな（あくまで竹が正）。
...
\`\`\`json
{
  ...
  "estimatedScale": "推定規模。★構造と築年数を必ず含めろ（指定があればその値、無ければ推察して『（推定）』付きで）。例: '木造2階建て 30坪 築30年'、'（推定）RC造 築25年前後 施工面積13.3m²'等",
...
大阪エリアの2025-2026年相場で見積もってください。自己検証ルールCを必ず実行してから出力すること。`
    ...
    try {
      ...
      try {
        const inputAge = Number(String(buildingAge ?? '').trim());
        const guessed = String(estimateResult.estimatedScale || '').match(/築\s*(\d{1,3})\s*年/);
        const resolvedAge = inputAge > 0 ? inputAge : (guessed ? Number(guessed[1]) : 0);
        if (resolvedAge > 0 && resolvedAge < 200) estimateResult.buildingAge = resolvedAge;
        const resolvedStructure = String(structure || '').trim()
...
## ★窓まわり・開口部の扱い★
- **見込み（ちり）への巻き込み: ${F.returnDepth}mm**
${F.returnDepth > 0
  ? `  → 開口1箇所ごとに「(幅 ＋ 高さ×2) × ${F.returnDepth}mm」を巻き込み面積として**壁面積に加算**しろ（窓は下端が窓台なので下辺は含めない。ドアは3辺で数える）。加算したことを formula に残せ。`
  : `  → **0なので、巻き込みは拾うな。**開口は控除するだけでよい。`}
- **下がり壁・垂れ壁**: 天井高の違う部分（CH表記のある下がり天井、吊戸棚下、掃出し窓上の垂れ壁、間仕切りの垂れ壁）は**壁面積に含めろ**。
  平面図だけで下がり寸法が読めないときは、推測せず unreadable に「下がり壁の高さが確定できない。断面図・展開図があれば拾えます」と書け。
- **カーテンボックスの内部**: ${F.curtainBox > 0 ? '**貼る**ものとして面積に含めろ。' : '**貼らない**ものとして数量に入れるな。'}
- **窓まわりの切り無駄**はロス率に含める。別立てで足すな（二重計上になる）。
...
## ★★資料が複数枚あるとき（図面＋数量表が一緒に来る場合）★★
ゼネコンからは「図面PDF」と「内訳書・材料一覧のPDF」が**一緒に送られてくることが多い**。次の順でやれ。
1. **まず1枚ずつ、それが図面か表かを判定しろ。**判定結果は drawingTypes に資料ごとに書け
   （例: ["1階平面図", "内訳書"]）。1つの資料の中に図面と表が両方あることもある。
2. **同じ部位が両方にあるときは、表の数量を採用しろ。**図面から拾い直して別行にするな（二重計上になる）。
   その行の source に「資料2 内訳書 12行目（図面と一致を確認）」のように**どちらから取ったか**を必ず書け。
3. **表に載っていない部位は、図面から拾え。**表があるからといって図面を見ないのは拾い落としだ。
   表が一部の工種（建具だけ・床だけ等）しか含まないことは普通にある。
...
### 断熱材（矩計図・仕様書・平面図の凡例）
- **部位ごとに分けて拾え**: 床（基礎断熱／床下断熱）・外壁（充填／付加）・天井／屋根。まとめて1行にするな。
- 単位は **㎡**。**厚み（t=105、t=155 等）と種別を name に必ず入れろ。**
  種別例: グラスウール16K/24K、ロックウール、押出法ポリスチレンフォーム3種b、フェノールフォーム、現場発泡ウレタン、セルロースファイバー。
  **同じ㎡でも厚みと種別で単価が数倍違う**ので、仕様書・矩計図に書いてあるものを落とすな。
- 数量の出し方:
  - 床断熱 = 床面積（玄関土間・浴室ユニット下など断熱しない部分は引く）
  - 外壁充填 = 外壁面積 −（1㎡以上の窓・出入口を控除）
  - 天井断熱 = 天井面積 ／ 屋根断熱（勾配天井・屋根断熱工法）= 屋根の実面積
- 吹込み・現場発泡は㎡ではなく **m3** で拾うことがある。図面・仕様書の単位表記に合わせ、どちらで拾ったかを unit に明記しろ。
- ロス率は 板状・マット状5%／現場発泡0%。
- **図面・仕様書に断熱の記載が無ければ推測で足すな。** unreadable に「断熱仕様の記載が無い。矩計図・仕様書があれば拾えます」と書け。
...
- **箇所数・台数で拾うもの**（面積で出すな）:
  ...
  - 照明器具（ベースライト・ダウンライト・ブラケット・非常灯・誘導灯・高天井灯）
...
1. **寸法数値が最優先**。図面に寸法線の数値（例 8,190）があれば必ずそれを使え。縮尺からの目測は、寸法数値が無い部位でだけ使い、その行の confidence を「低」にしろ。
   ...
   図面の余白・引き出し線の先・室内の注記を隅々まで見て、**書いてある面積を1つも取りこぼすな。**
   書いてある面積を使った行は confidence「高」だ。
...
9-0. ★★**確認申請図の面積表（A/L/V/S）を読み違えるな**★★
   実施図・確認申請図の下部には、室ごとに次の4行が並んだ表がよく付いている。**凡例に定義が書いてある。**
   - **A ＝ 居室（又は室）の床面積** ←★拾い出しに使うのはこれだけ★
   - L ＝ 有効採光面積（採光計算）／ V ＝ 有効換気面積（換気計算）／ S ＝ 有効排煙面積（排煙計算）
   **L・V・S は建築基準法の計算値であって、部屋の面積ではない。**これを別の室の床面積として拾うと、
   ありもしない部屋が3つ増えて数量が大きく水増しされる（実例: 食堂・廊下 A123.15 の L25.80・V9.90 を
   「廊下A」「廊下B」という別室として拾ってしまった）。
   **表の中で A の行だけを見ろ。**Lk・Lw・Lh・Sw・Sh も同じく計算値だ。使うな。
   同様に、耐火・防火の記号（FP060NP など）、建具記号（AW-3・AD・SD・WD）、
   採光補正係数（Lk=…）、隣地境界線の距離（16.82 等）を面積や数量と間違えるな。
...
## ★★部位名・行名・単位は下の表で固定しろ（バラバラだと機械が集計できない）★★
| 拾うもの | part | name の書き出し | unit |
|---|---|---|---|
| 床の仕上げ | 床 | 「床仕上げ ― <室名>」 | ㎡ |
| 天井の仕上げ | 天井 | 「天井仕上げ ― <室名>」 | ㎡ |
| 壁の仕上げ（クロス等） | 内壁 | 「壁仕上げ ― <室名>」 | ㎡ |
| 壁の下地（LGS・ボード） | 内壁 | 「壁下地 ― <室名>」 | ㎡ |
| 天井の下地 | 天井 | 「天井下地 ― <室名>」 | ㎡ |
| 幅木・巾木 | 内壁 | 「幅木 ― <室名>」 | **m** |
...
11. **同じ数量を二重に出すな。**「居室1〜10」を1行にまとめるのは構わないが、
    **各室の行とそれをまとめた行の両方を出すな**（そのまま金額に乗って二重計上になる）。
    どちらか一方にしろ。資料全体の総計は items ではなく summary に書け。
12. **quantity が空（null）や 0 の行を出すな。**数量が読めなかったなら、その行は items に入れず unreadable に回せ。
    数量の無い明細行は、金額の段階で0円になって黙って消える。
    **items に入れなかった室は、差引きの引き算にも含めるな**（上の「差引き」の項を守れ）。

## ★★「推測するな」と「根拠を書いて算出しろ」は別物★★
鉄則5で禁じているのは**根拠なく数字を作ること**だ。**式と仮定を書いた算出は禁止していない。**
図面に書いていないからと諦めて空にするより、根拠を明示して出すほうが役に立つ。次の3つは必ずやれ:

1. **廊下・ホールなど、面積の表記が無い部分は「差引き」で1行立てろ。**
   ...
   **各室を挙げただけで終わるな。**床の合計が指定面積に足りないまま出すのが、一番多い外し方だ。
   ...
   ★**屋外は入れるな。**テラス・ポーチ・野外ステージ・光庭・バルコニーは内装の対象外だ。
   図面に面積が書かれていればその分を明示的に引き、書かれていなければ差引き行の note に
   「屋外部分を含む可能性あり」と書いて confidence を下げろ。
2. **壁・天井の面積は、展開図が無くても概算しろ。**
   壁 = 各室の内周長の合計 × 天井高（内周長 ≒ 4×√その室の床面積。細長い室は縦横から）。
   天井 = 床面積（仕上げをしない部分は引く）。
   formula にその式を、assumption に「天井高◯mと仮定」と書き、confidence は「低」にしろ。
3. **幅木は「壁の延長 × 0.9（開口控除）」で出せ。**壁延長 = 壁面積 ÷ 天井高。
4. **★床で挙げた区画は、壁・天井・幅木でも必ず挙げろ。★**
   床に出てくる室が壁に出てこない、というのは拾い落としだ。**小さい室（トイレ・脱衣・倉庫・前室など）ほど
   周長の割に面積が小さく、床の割に壁面積が大きくなる。**面倒でも省くな。まとめて1行にしてもよいが、抜かすな。
5. **廊下・ホールに 4×√面積 を使うな。**細長いので周長を大きく外す。
   廊下は「延長 × 2面 × 天井高」で出せ。延長が図面から読めなければ、**面積 ÷ 有効幅**で延長を出せ。
   ★**有効幅は図面に書いてある数字を使え。**「有効幅1,700」とあれば 1.7m だ。**勝手に1.8mと仮定するな。**
   書いていないときだけ仮定してよく、その場合は assumption に「有効幅◯mと仮定」と明記しろ。
   formula にその式を書け。
4. **★床で挙げた区画は、壁・天井・幅木でも必ず挙げろ。★**
   床に出てくる室が壁に出てこない、というのは拾い落としだ。**小さい室（トイレ・脱衣・倉庫・前室など）ほど
   周長の割に面積が小さく、壁面積は大きくなる。**面倒でも省くな。まとめて1行にしてもよいが、抜かすな。
5. **廊下・ホールに 4×√面積 を使うな。**細長いので周長を大きく外す。
   廊下は「延長 × 2面 × 天井高」で出せ。延長が図面から読めなければ、**面積 ÷ 有効幅**で延長を出せ
   （有効幅は図面に「有効幅1,700」等と書かれていることが多い）。formula にその式を書け。

それでも根拠が立たないものだけを unreadable に回せ。

## ★★手順（この順でやれ。いきなり items を書き始めるな）★★
1. **まず図面上の室名を全部書き出せ。**平面図を端から端まで見て、室名の文字を1つ残らず拾う
   （居室・食堂・厨房・事務室・玄関・相談室・脱衣室・浴室・WC・PS・EV・階段・廊下・ポンプ室・洗濯室…）。
   **この時点では面積を考えるな。名前を数えるだけだ。**
2. 次に、**書き出した室を1つずつ**、面積を探す。探す場所は3つ:
   ①図面下部の面積表の **A 行** ②室名の横の注記（「厨房 A：4.50×3.425＝15.41m²」）
   ③寸法線から自分で計算（縦×横）。
3. **1で数えた室の数と、items に出した床の行の数を突き合わせろ。**
   数が合わなければ、面積が見つからなかった室がある。それを unreadable に書け。
   **室名を数えた時点で存在が分かっているのだから、「気づかなかった」で落とすな。**
4. そのうえで、壁・天井・幅木を同じ室について出せ。

## ★★出す前に検算しろ（ここを飛ばすと必ず抜ける）★★
- **部位ごとに、行の数量を足して総計を確かめろ。**
  例: 床の行を全部足す → 図面に記載された床面積と一致するか。
- **合わない場合は、拾い落とした部屋・部位がある。**もう一度図面を見て、抜けている室を足せ。
  それでも合わないなら、**差がいくらで、どの部分が読めなかったのかを unreadable に書け。**
  黙って少ない数量のまま出すな。客が電卓を当てれば一発で分かる。
- 部屋を1つずつ挙げるときは、**図面の端から順に、区画を飛ばさずに**見ていけ。
- **床の行と、壁・天井の行を突き合わせろ。**床にあって壁に無い区画があれば、それは拾い落としだ。
  出す前に「床で挙げた区画名」を並べ、壁・天井・幅木にも同じ区画が入っているか1つずつ確かめろ。
- ★**壁の総面積 ÷ 床面積 を必ず計算し、1.8〜3.2 に収まるまで直してから出せ。**★
  内装の実務ではこの範囲に必ず入る（大部屋主体で1.8〜2.2、個室主体で2.6〜3.2）。
  **3.2超＝廊下を長く見すぎ（÷有効幅の使いすぎ）。1.8未満＝室を拾い落としたか、細長い部分を丸めすぎ。**
  範囲に入っていない答えは出すな。warnings に「壁/床比 ◯.◯倍」と実際の値を必ず書け。
- **差引きの残りを、まるごと「面積 ÷ 廊下の有効幅」で長さに直すな。**
  差引きには廊下だけでなく、玄関・ホール・階段室・EV・屋外テラス等が混ざっている。
  細長い廊下として扱ってよいのは差引きの一部だけだ。**まとまった空間は 4×√面積 で周長を出せ。**
  区別がつかないときは、**差引きの半分を細長い廊下（÷有効幅）、半分をまとまった空間（4×√面積）**として按分しろ。
...
- 屋根面積を測る基準は**屋根の上にあるもの**に限れ。地上の車・人・塀は使うな。
- 参照物は**なるべく大きいもの**を選べ。小さい物を基準にすると誤差が拡大する。
- 屋根の上に基準が1つも無いなら、面積は確定しない（下の判定へ）。

### ★基準にしてはいけないもの（寸法が規格化されていない）
実測で、これらを基準にした回答は大きく外れた。使うな。
- **換気フード・ベンチレーター・換気扇**（直径は製品によって250〜800mmとばらつく）
- **ドーマー窓・天窓**（寸法は設計次第）
- **雪止め金具の取付ピッチ**（勾配と積雪量で変わる）
- **棟包みの幅**（板金の納まり次第）
使ってよいのは、下の表にある**働き幅・働き長・ピッチが規格で決まっている屋根材**と、
太陽光パネル（1,650×990mm）だけだ。屋根材のピッチが読めないほど遠い写真なら、
面積は確定しないと判断しろ。

## ★★数えるな★★
規格材を「何枚・何山あるか数えて掛ける」方法は使ってはならない。
画像モデルは4〜6個を超える反復物体を正確に数えられず、重なりがあると精度が3〜4割落ちる。
[arXiv:2407.06581 "Vision language models are blind"]
- 正しいやり方は、**基準物1つの見かけの長さと、屋根の見かけの長さの比**をとること。
  例: 「パネルの短辺(0.99m)が屋根の幅のおよそ1/12 → 屋根幅 ≒ 11.9m」
- 「山を数えたら12本あった」ではなく「山ピッチ0.5mが屋根幅の約1/24に見える」と考えろ。

## 写真の中にある「寸法がわかっているもの」（基準スケール）
| 対象 | 実寸 |
|---|---|
| 太陽光パネル（住宅用・標準） | 1,650 × 990mm。屋根の上にあるので最良の定規 |
| 折板屋根の山ピッチ（働き幅） | 重ね式・ハゼ締め式 500mm / 嵌合式 600mm |
| 折板の山高 | 88mm（一般）/ 150mm（大スパン）← 展開係数が変わるので必ず見分けろ |
| 母屋・タイトフレームの間隔 | 1,200〜1,500mm |
| スレート大波 | 働き幅 950mm、波ピッチ 130mm |
| 化粧スレート（カラーベスト） | 働き幅 910mm、働き長 182mm |
| 和瓦（J形53A） | 働き幅 235mm × 働き長 235mm |
| 換気棟・棟包み | 長さは棟の全長。幅 300mm前後 |
| 人（成人） | 身長 1,700mm ※**屋根の上に立っている人だけ**基準に使える |
| 普通乗用車 | 全長 4,500mm ※地上。**間口の目安にのみ使い、屋根面には当てるな** |

## 屋根面積の出し方（この順に計算しろ）
1. **基準スケールの同定**: 屋根平面上の、なるべく大きい規格物を1つ選ぶ。scaleRef に必ず書く。
2. **見かけの比で寸法を出す**: 基準物の実寸 ÷ 見かけの長さ × 屋根の見かけの長さ。数えるな。
3. **面ごとに測れ**: 寄棟・入母屋は屋根面を1面ずつ測って合算する。面積比が使えるのは
   単一平面の中だけだ。[Criminisi 2000]
4. **軒の出を足す**: 屋根は外壁より外に出ている。**軒の出 450〜600mm、けらば 300〜600mm**
   を四周に加える。無視すると一般住宅で約9m²小さくなる。
   水平投影 = (間口 + 2×けらばの出) × (桁行 + 2×軒の出)
5. **勾配補正**: 実面積 = 水平投影 × 勾配係数。係数 = √(1+(寸/10)²)。
   | 勾配 | 係数 | 勾配 | 係数 |
   |---|---|---|---|
   | 1寸 | 1.005 | 5寸 | 1.118 |
   | 2寸 | 1.020 | 6寸 | 1.166 |
   | 3寸 | 1.044 | 7寸 | 1.221 |
   | 3.5寸 | 1.059 | 8寸 | 1.281 |
   | 4寸 | 1.077 | 10寸(矩) | 1.414 |
   ・勾配が読めないときの既定値: 戸建て **4〜5寸(1.077〜1.118)**、折板屋根 **0.5〜1寸(1.005)**。
   ・写真だけからの勾配判定は誤差が大きい。断定せず既定値を使い、basis にそう書け。
6. **展開係数（折板・波板のみ）**: 山谷のぶん、張る材料の面積は屋根面積より大きい。
   ・折板 **山高88mm → 1.41**（自社実績。見積書に「折板屋根面積×1.4」と明記）
   ・折板 **山高150mm → 1.69**
   ・★山高を確認せずに1.69を使うな。88mmに1.69を当てると約21%過大になる。
   ・スレート大波 1.1 / 平葺き・瓦・シングル 1.0
   これを掛けたものが**見積の数量**だ。屋根面積そのものを数量にするな。

## 戸建て住宅は「床面積」から積むほうが確実
写真より、施主が知っている床面積のほうが正確。
**屋根の水平投影 ＝ 1階の建築面積 ＋ 軒の出のぶん**、**屋根面積 ＝ 水平投影 × 勾配係数**。
・総2階なら 1階床面積 ＝ 延床面積 ÷ 2。平屋なら 1階床面積 ＝ 延床面積。
・目安として1階床面積の 1.25〜1.45倍に収まる（軒0.45〜0.6m・勾配4〜5寸のとき）。
  この倍率は幾何から出したもので、内訳（軒の出・勾配）を basis に必ず書け。
・切妻でも寄棟でも、同じ建築面積なら屋根面積はほぼ同じ（水平投影が同じ）。
・敷地面積 × 建ぺい率 は建築面積の**上限**であって実際の建築面積ではない。断定に使うな。

## ★最初に必ず判定しろ: この写真で面積が「確定」するか
屋根の輪郭が写真に収まっていなければ面積は確定しない。だが**必ず推測値を出せ**。
数字が無いと相手は次に進めない。「分かりません」で終わらせるな。
- 屋根の上に立って撮った写真、屋根面の手前だけの写真、奥や左右の端が切れている写真
  → **coversWholeRoof = false**。それでも quantityM2 に**最も確からしい推測値**を入れろ。
  rangeMinM2 / rangeMaxM2 に「必ずこの間に収まる」レンジを入れろ。
  askUserFor に、**1つ聞けば面積が確定する情報**を書け。
  ・戸建て住宅なら「1階の床面積（坪でも㎡でも可）」または「延床面積と階数」が最短。
  ・工場・倉庫なら「間口(m)」「桁行(m)」。折板の山ピッチから間口は読めることが多いので、
    その場合は桁行だけを聞け。
- 屋根の四隅・棟から軒までが1枚に収まっている → coversWholeRoof = true。確定値を出せ。

## 推測値の作り方（写真で確定しないとき）
**読めるところは読み、読めない方向だけ標準値で補え。** 全部を勘で置くな。
1. 写真から読める寸法は必ず使う（基準物との見かけの比で出す）。
2. 読めない方向だけ下の標準値を使い、使った標準値を basis に必ず書く。
   ・工場・倉庫の桁行: 1スパン6.0mが標準。中小工場は 18〜36m が多い。
   ・工場・倉庫の間口（梁間）: 小規模 8〜12m / 中規模 15〜20m / 大規模 25〜30m。
   ・戸建て住宅の1階床面積: 25〜35坪（83〜116m²）が最頻。
3. レンジは「標準値の下限×読めた寸法」〜「上限×読めた寸法」で作る。狭く見せかけるな。
4. confidence は「低」。basis の冒頭に「【推測】」と書け。

★禁止: 根拠を書かずに 600m²・1000m²・1200m² のような丸い数字だけを置くこと。
  どの寸法をどの基準物から読み、どこに標準値を当てたのかを basis に必ず書け。

## 注意
- 写真は透視投影。手前が大きく奥が小さい。奥行きの見かけの長さは実長より短いので、
  見かけの比のまま計算すると**面積を小さく見積もる**。消失点方向の縮みを考慮しろ。
...
slopeFactor と developFactor は内装では常に 1 だ。`;
  ...
  // ── 面積の較正を実測から学習する ────────────────────────────────
  // AIは寸法を系統的に小さく答える。その補正係数 AREA_CALIBRATION_BASE=1.30 は
  ...
  //
  // 2026-07-12 再フィット: ハーネス(tools/harness-roof)で再ベンチしたところ、1.40は生比の
  // 幾何平均を1.11（+11%過大）に押し上げていた。中央最適は1.22だが、面積の過小は「安く見積もる=
  // 赤字」で過大より痛いので、無バイアス寄り＋わずかな安全マージンとして 1.30 に下げた。
...
下の target 別の手順に従い、**いつもどおり最も確からしい寸法を出せ**。

※ ここに「写真では確定できないので概算でよい」といった趣旨を書かないこと。
   「写真は概算として扱う」のは製品側の判断であり、main.ts の後処理で強制している。
   プロンプトの仕事は**寸法をできるだけ正確に読ませること**だけで、精度の限界を
   AIに教える必要はない（自信を下げる指示が読み取り精度に効く可能性がある）。
   一度この趣旨の文言を入れて計測したが、実行ごとのばらつきが大きく（下記）、
   良し悪しを判定できなかった。効果が確かめられない指示は入れない、という判断。

※★写真からの推定は再現しない。同じ画像・同じプロンプト・同じモデルで2回回すと、
   11件中3件が1.5倍以上ずれた（最悪 0.29倍＝3.4倍の開き／ばらつき±54%）。
   2026-09-02 計測・app/tools/harness-area/。
   → プロンプトを直したときは、必ず複数回回して比べること。1回の結果で
     良くなった悪くなったを判断すると、ノイズを見て意思決定することになる。
...
以下のJSONのみを返してください（説明文は不要）:
{
  ...
  "confidence": "高/中/低"
    ...
    // ── 面積の組み立てはコード側でやる ──────────────────────────
    // 正解付き11件（住宅7・工場倉庫4、うち空撮8）のベンチマークで、6通りのプロンプトを比較した。
    // AIに面積そのものを答えさせるより、寸法だけ答えさせてこちらで掛けるほうが明確に精度が高い。
    //   寸法のみ+較正: 1.25倍以内 8/11、最悪 1.81倍
    //   面積を直接:    1.25倍以内 3/11
    //   旧実装:        1.25倍以内 6/11、ただし最悪 4.17倍（2000㎡に8000㎡と回答）
    // Crop&Zoom(2パス)も複数回サンプリングの中央値も、この課題では効かなかった（むしろ悪化）。
    //
    // AREA_CALIBRATION: どのプロンプトでもAIは寸法を系統的に小さく答える。
    // 住宅0.76 / 工場0.77 / 倉庫0.86 と層をまたいでほぼ一定なので、定数で補正する。
    // 1.40 は11件のベンチで 1.25倍以内7件・|log比|中央0.196・最悪2.16倍と最良だった値。
    // ★正直な但し書き: n=11 では 1.40 と 1.49 の差は誤差の範囲（1.49だと幾何平均比が
    //   ちょうど1.00になるが、1.25倍以内は6件に減る）。Leave-One-Out で係数ごと選び直すと
