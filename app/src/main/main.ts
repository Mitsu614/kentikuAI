...
金額は数値のみ（カンマや円記号は除去）。日付はYYYY-MM-DD形式に変換。`
  ...
  const analyzeImageCore = async (data: any) => {
    let { imageBase64, beforeImage, afterImage, comment, location, area, clientAttrs, roofType, structure, buildingAge } = typeof data === 'string' ? { imageBase64: data, beforeImage: null, afterImage: null, comment: '', location: '', area: '', clientAttrs: null, roofType: null, structure: '', buildingAge: '' } : data;
