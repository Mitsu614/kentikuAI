...
const sections = [
  ...
  {
    // ライトプランのお客様は取り込み代行が付かず、ご自身で学習させていただく。
    // 「どうやって覚えさせるのか」が分からないまま3ヶ月経つと、精度が上がらないまま解約になる。
    title: 'AIに自社の単価を覚えさせる',
    icon: '🎓',
    steps: [
      '①過去の見積書を取り込む … 「紙を電子化（OCR）」で、過去に出した見積書のPDFや写真を読み込む。コメント欄に工事の内容を書いておくと、次から似た工事でその単価が使われます',
      '②AIが出した単価を直す … 「AI 見積もり」で出てきた金額が違うときは、その場で直して保存。直した内容をAIが覚えます（学習のための特別な入力は不要です）',
      '③実際にかかった金額を入れる … 「利益レポート・予実管理」で実績を入力すると、見積と実際の差をAIが学びます。ここが一番効きます',
      '④拾い出しの数量を直す … 図面から拾った数量が違うときは直す。数量の拾い方だけは全社で共有され、金額は共有されません',
      '目安は20〜30件 … これくらい入れていただくと、御社の単価に寄ってきます。使い始めが一番精度が低く、3か月後が一番賢い状態です',
      '学習が進むとメールでお知らせします（1日1通まで）',
    ],
  },
  {
    title: '写真から見積',
